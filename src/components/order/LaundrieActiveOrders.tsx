    ...
    <div className="px-4 py-20 max-w-[500px] mx-auto">
      {laundrie?.data?.length === 0 ? (
        ...
        <div className="space-y-4">
          {laundrie?.data?.map((order) => (
            <OrderCard key={order.id} order={order} />
          ))}
          <div className="w-full">
            <Link
              href={PAGES.ORDER}
              className="bg-primary px-8 py-3 text-primary-foreground hover:bg-primary/90 w-full  mx-auto flex items-center justify-center"
            >
              Place an order
            </Link>
          </div>

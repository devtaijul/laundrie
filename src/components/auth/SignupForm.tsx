...
export function SignupForm({ referralCode }: { referralCode?: string }) {
  ...
  const renderEmailStep = () => (
    <div className="space-y-6">
      ...
      {referralCode && (
        <div className="flex items-center gap-2 rounded-lg bg-primary/10 border border-primary/20 px-4 py-3 text-sm text-primary font-medium">
          <Gift className="h-4 w-4 shrink-0" />
          <span>
            Referral code applied — you&apos;ll get{" "}
            {referralCode.startsWith("20")
              ? formatMoney(20)
              : formatMoney(10)}{" "}

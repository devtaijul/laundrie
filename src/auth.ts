...
export const { handlers, signIn, signOut, auth } = NextAuth(async () => {
  ...
  try {
    const authSetting = await prisma.authSetting.findFirst({
      select: {
    ...
    googleClientId = authSetting?.google_client_id?.trim() ?? "";
    googleClientSecret = authSetting?.google_client_secret?.trim() ?? "";
  } catch {
    googleClientId = "";
    googleClientSecret = "";
  }
    ...
    callbacks: {
      ...
      async jwt({ token, user, account, trigger, session }) {
        ...
        if (trigger === "update" && session?.user) {
          ...
          token.useType = (session.user as SafeUser)
            .useType as $Enums.TypeOfUse;
        }
